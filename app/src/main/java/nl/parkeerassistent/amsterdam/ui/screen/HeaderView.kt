            ...
            if (loggedIn) {
                Box(Modifier.align(Alignment.CenterEnd)) {
                    ...
                    DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
                        DropdownMenuItem(text = { Text(stringResource(R.string.parking_history)) }, onClick = { expanded = false; onHistory() })
                        DropdownMenuItem(text = { Text(stringResource(R.string.user_add_balance)) }, onClick = { expanded = false; onPayment() })
                        DropdownMenuItem(text = { Text(stringResource(R.string.account_header)) }, onClick = { expanded = false; onAccounts() })
                        DropdownMenuItem(text = { Text(stringResource(R.string.settings_header)) }, onClick = { expanded = false; onSettings() })
                        DropdownMenuItem(text = { Text(stringResource(R.string.login_logout)) }, onClick = { expanded = false; onLogout() })
                    }
                }
            }
        }
